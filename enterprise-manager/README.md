...
A docker (https://www.docker.com/) image build for the APM solution of CA Technologies (CA APM). This project build a docker image for an Introscope enterprise manager in version 10.2. It is meant to be used in combination with the docker image ``apm-postgres`` which provides the database for the enterprise manager.
...
The docker image needs the installation files from CA Wily Introscope. Simply copy the files ``introscope10.2.0.27otherUnix.jar`` (installer of the enterprise manager).
